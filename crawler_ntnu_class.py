...
'''好的，我已經接收並記住了第六個檔案的內容：crawler_ntnu_class.py。

🎓 crawler_ntnu_class.py 程式碼摘要：
這個檔案是針對「國立師大美術館」的專用爬蟲類別。它採用了結合傳統爬蟲和 Gemini 結構化提取的混合策略，來處理展覽日期、概述和場地這類通常嵌在文本中的資訊：

資料模型：延續 exhibition_data dataclass，並將 hallname 設為 '國立師大美術館'。

提取策略（混合 E + T）：

I. 爬取固定資訊 (_get_addrandtimeinterval_info)：

訪問主頁，直接從固定區塊提取美術館的地址 (addr) 和開放時間 (visit_time_interval)。

II. 基礎提取 (_extract_base_info)：

遍歷展覽列表，提取展覽標題、大圖 URL 和詳細頁面 URL。

關鍵步驟：訪問每個展覽的詳細頁面，並將頁面內所有文本 (div#primary) 抓取下來，存入 data.pagetext。

將 I 步驟中爬取到的全館地址和開放時間直接賦予每個展覽實例。

III. AI 結構化提取 (_extract_with_gemini)：

用途：主要用於從上一步驟獲得的長篇網頁文本 (item.pagetext) 中，精確提取開始日期、結束日期、詳細概述和場地等資訊。

提示詞工程：提示詞指示 Gemini 進行多項複雜任務：

解析並輸出 ISO 8601 格式的日期 (YYYY-MM-DD)，若缺失則使用預設值 (1990-01-01 / 2050-12-31)。

提供至少 100 字的完整概述，並排除非敘事性文字。

比對已知的館舍地址 ({ADDR})，如果展覽場地包含該地址，則在 space 欄位中排除地址，僅保留場地名稱。

轉換 (Transform)：

地理編碼 (_transform_googlegeocoding)：使用 Google Maps Geocoding API 將地址和場地合併進行地理編碼。'''
